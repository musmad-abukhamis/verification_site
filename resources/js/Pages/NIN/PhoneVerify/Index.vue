...
const canSubmit = computed(() => /^0[0-9]{10}$/.test(form.value) && !form.processing);

const submit = () => {
    const routeName = provider.value === 'v1' ? 'nin.phone.v1' : 'nin.phone.v2';
...
<template>
    <Head title="NIN Verify by Phone" />
    <AuthenticatedLayout>
        <div class="space-y-6">
            <!-- Wallet -->
            <div class="bg-gradient-to-r from-blue-600 to-cyan-600 rounded-xl shadow p-6 text-white">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow p-6">
                <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-1">NIN Phone Verification</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-1">Look up a NIN record using a registered phone number — always billed at premium price.</p>
                <p class="text-sm font-semibold text-blue-600 dark:text-blue-400 mb-6">Price: ₦{{ price?.toLocaleString() }}</p>
                ...
                <div class="flex gap-2 mb-6">
                    <button v-for="v in ['v1', 'v2']" :key="v" @click="provider = v"
                        :class="['px-5 py-2 rounded-lg text-sm font-semibold transition-colors', provider === v ? 'bg-blue-600 text-white shadow' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200']">
                ...
                <form @submit.prevent="submit" class="space-y-4 max-w-lg">
                    <div>
                        ...
                        <input v-model="form.value" type="tel" maxlength="11" placeholder="e.g. 08012345678"
                            class="w-full rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                        <p class="mt-1 text-xs text-gray-500">Nigerian phone number starting with 0 (11 digits)</p>
                    ...
                    <div>
                        ...
                        <input v-model="form.ref" type="text" placeholder="Custom reference"
                            class="w-full rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                    </div>
                    <button type="submit" :disabled="!canSubmit"
                        class="flex items-center gap-2 px-6 py-2.5 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                        <svg v-if="form.processing" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
                        </svg>
                        {{ form.processing ? 'Looking up...' : `Verify (₦${price?.toLocaleString()})` }}
                    </button>
                </form>
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow">
                <div class="p-4 border-b border-gray-200 dark:border-gray-700">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Recent Phone Lookups</h3>
                </div>
                <div class="overflow-x-auto">
                    ...
                    <div v-if="transactionsList.length === 0" class="py-10 text-center text-gray-400">No phone lookups yet</div>
                </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                <div class="p-6">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-bold text-gray-900 dark:text-white">Phone Lookup Result</h3>
                    ...
                    <div v-if="verificationResult?.photo" class="flex justify-center mb-4">
                        <img :src="verificationResult.photo" alt="Photo" class="w-24 h-24 rounded-full object-cover border-4 border-blue-200" />
                    </div>
                    <div class="space-y-2">
                    ...
                    <button @click="closeResult" class="mt-6 w-full bg-blue-600 text-white py-2.5 rounded-lg hover:bg-blue-700 font-medium">Close</button>
                </div>
            </div>
        </div>
    </AuthenticatedLayout>
