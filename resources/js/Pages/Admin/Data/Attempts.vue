...
const applyFilters = () => router.get(route('admin.data-attempts.index'), filters, {
    preserveState: true,
    replace: true,
});
...
<template>
    <Head title="Data Vendor Calls" />

    <AdminLayout>
        <div class="space-y-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Vendor Calls</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400">
                    Every hop, including the ones that failed and were failed over — where a quietly broken vendor shows up.
                </p>
            </div>

            <!-- 24h health per vendor -->
            <div v-if="summary.length" class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <div v-for="row in summary" :key="row.vendor" class="rounded-lg bg-white p-4 shadow dark:bg-gray-800">
                    <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ row.vendor }}</p>
            ...
            <div class="grid gap-3 rounded-lg bg-white p-4 shadow dark:bg-gray-800 sm:grid-cols-4">
                <select v-model="filters.network" @change="applyFilters" class="rounded-lg border-gray-300 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100">
                    <option value="">All networks</option>
                    <option v-for="n in networks" :key="n" :value="n">{{ n.toUpperCase() }}</option>
                </select>
                <select v-model="filters.vendor_id" @change="applyFilters" class="rounded-lg border-gray-300 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100">
                ...
                <input v-model="filters.search" @keyup.enter="applyFilters" placeholder="Reference or phone…"
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm dark:divide-gray-700">
                        <template v-for="a in attempts.data" :key="a.id">
                            <tr>
                                ...
                                <td class="px-4 py-3 text-gray-700 dark:text-gray-300">
                                    {{ a.plan ?? '—' }}
                                    <span class="block text-xs text-gray-400">{{ a.network?.toUpperCase() }} · {{ a.phone }}</span>
                                </td>
                                <td class="px-4 py-3 text-gray-700 dark:text-gray-300">{{ a.vendor }}</td>
                            ...
                            <tr v-if="expanded === a.id">
                                <td colspan="7" class="bg-gray-50 px-4 py-4 dark:bg-gray-900/40">
                                    <div class="mb-3 flex flex-wrap gap-4 text-xs text-gray-500">
                                        <span>Reference: <span class="font-mono text-gray-700 dark:text-gray-300">{{ a.reference }}</span></span>
                                        <span>Transaction status: <span class="font-medium text-gray-700 dark:text-gray-300">{{ a.txn_status ?? '—' }}</span></span>
                                    </div>
                        ...
                        <tr v-if="!attempts.data.length">
                            <td colspan="7" class="px-4 py-8 text-center text-gray-500">No vendor calls recorded yet.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
